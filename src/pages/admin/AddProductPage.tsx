...
const AddProductPage: React.FC = () => {
  ...
  const initPropertyValues = (props: CategoryPropertyGroupResponse) => {
    ...
    groups.forEach((prop) => {
      if (
        prop.hasDefaultOptions &&
        prop.defaultPropertyOptions &&
        prop.defaultPropertyOptions.length > 0
      ) {
  ...
  const renderPropertyGroup = (title: string, props: CategoryPropertyDTO[], required = false) => {
    ...
    return (
      <Card variant="outlined">
        <CardHeader
          title={title}
          subheader={required ? "Required when creating variant" : "Optional variant metadata"}
        />
        <CardContent>
